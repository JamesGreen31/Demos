...
function operationToText(operation, index) {
  const step = index + 1;
  const action = operation.type === 'recycle' ? 'Recycle' : 'Craft';
  const target = operation.type === 'recycle' ? operation.itemName : operation.output;
  return `#${step} ${action} ${target} (Δ stacks ${operation.stackDelta.toFixed(2)}, cost ${operation.cost.toFixed(0)})`;
}
    ...
    <main className="min-h-screen p-6 md:p-10 bg-slate-50 text-slate-900">
      <div className="max-w-6xl mx-auto space-y-6">
        <header className="space-y-2">
          ...
          <p className="text-sm text-slate-600">Compress inventory from CSV input using the active crafting/recycling dataset, including multi-phase craft chains.</p>
        </header>
          ...
          <>
            <section className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-2">
              ...
              <div className="rounded-md border border-slate-200 bg-slate-50 p-3">
                <p className="text-sm font-semibold">Crafting order</p>
                {result.aggressiveOps.length ? (
                  <ol className="mt-2 space-y-1 text-xs text-slate-700">
                    {result.aggressiveOps.map((operation, index) => (
                      <li key={`${operation.id}-${index}`}>{operationToText(operation, index)}</li>
                    ))}
                  </ol>
                ) : (
                  <p className="mt-2 text-xs text-slate-600">No compression actions available from this input.</p>
                )}
              </div>
                  ...
                  <article key={bucket.threshold} className="border border-slate-200 rounded-lg p-3 bg-slate-50">
                    ...
                    {bucket.operations.length > 0 && (
                      <details className="mt-2 text-xs text-slate-700">
                        <summary className="cursor-pointer font-semibold">Show crafting order</summary>
                        <ol className="mt-2 space-y-1">
                          {bucket.operations.map((operation, index) => (
                            <li key={`${bucket.threshold}-${operation.id}-${index}`}>{operationToText(operation, index)}</li>
                          ))}
                        </ol>
                      </details>
                    )}
